...
class DB_Manager:
    ...
    def make_pretty(self):
        ...
        if answer == 'y':
            xml = minidom.parse(self._seller_db) # or xml.dom.minidom.parseString(xml_string)
            pretty_xml_as_string = xml.toprettyxml()

            pretty_db = open(self._seller_db+str(int(time.time())),'w')
            pretty_db.write(pretty_xml_as_string)
            pretty_db.close()
            print "DB pretty print done."
        else:
            print "Good decsision. Bye!"

    def get_seler_info(self, seller_id):
        product_list = {}
